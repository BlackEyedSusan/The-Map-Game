...
def room(game_id):
    if request.method == "POST":
        ...
        if len(empire) < 1:
            flash('You cannot leave the empire name blank.')
        if len(empire) > 200:
    ...
    for empire in db.session.query(Empires).filter_by(game=id):
        ...
        print(empire.name)
    empire_key = [empires]
    return render_template("room.html", user=current_user, players = players_output, game = name, game_id = id, empire_key=empires)
